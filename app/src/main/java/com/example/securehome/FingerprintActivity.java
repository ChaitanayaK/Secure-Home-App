...
public class FingerprintActivity extends AppCompatActivity {
    ...
    private static final String KEY_NAME = "SECUREHOME";
    ...
    private void getFingerprint(){
        ...
        errorText = (TextView) findViewById(R.id.textMsg);

        // Here, we are using various security checks
        // Checking device is inbuilt with fingerprint sensor or not
        if(!fingerprintManager.isHardwareDetected()){
            Toast.makeText(this, "Device does not support fingerprint sensor", Toast.LENGTH_SHORT).show();
            finish();
            ...
            if (ActivityCompat.checkSelfPermission(FingerprintActivity.this, Manifest.permission.USE_FINGERPRINT) != PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(this, "Fingerprint authentication is not enabled", Toast.LENGTH_SHORT).show();
                finish();
            }else{
                // Check for at least one registered finger
                if (!fingerprintManager.hasEnrolledFingerprints()) {
                    errorText.setText("Register at least one finger");
                    Toast.makeText(this, "Register at least one finger", Toast.LENGTH_SHORT).show();
                    finish();
                }else{
                    // Checking for screen lock security
                    if (!keyguardManager.isKeyguardSecure()) {
                        errorText.setText("Screen lock security not enabled");
                        Toast.makeText(this, "Screen lock security not enabled", Toast.LENGTH_SHORT).show();
                        finish();
                    }else{
                        ...
                        if (cipherInit()) {
                            ...
                            helper.Authentication(fingerprintManager, cryptoObject, key);
                        }
                    }
                }
            }
